...
def month_calendar_kb(year: int, month: int) -> InlineKeyboardMarkup:
    ...
    rows.append([
        InlineKeyboardButton(text=weekday, callback_data="noop")
        for weekday in ["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"]
    ])

    cells = [None] * first.weekday()
    cells += [date(year, month, day) for day in range(1, last.day + 1)]
    while len(cells) % 7:
        cells.append(None)

    for i in range(0, len(cells), 7):
        keyboard_row = []
        for day_value in cells[i:i + 7]:
            if day_value is None or day_value < start or day_value > end:
                keyboard_row.append(InlineKeyboardButton(text="·", callback_data="noop"))
            else:
                keyboard_row.append(InlineKeyboardButton(
                    text=str(day_value.day),
                    callback_data=f"date:{day_value.isoformat()}",
                ))
    ...
    competition_start_month = date(start.year, start.month, 1)
    competition_end_month = date(end.year, end.month, 1)

    if prev_month_first_day >= competition_start_month:
        nav.append(InlineKeyboardButton(
            text="◀️",
            callback_data=f"cal:{prev_month_first_day.year}-{prev_month_first_day.month:02d}",
        ))

    nav.append(InlineKeyboardButton(text="⬅️ В меню", callback_data="menu"))

    if next_month <= competition_end_month:
        nav.append(InlineKeyboardButton(
            text="▶️",
            callback_data=f"cal:{next_month.year}-{next_month.month:02d}",
        ))
...
def get_or_create_user(tg_user) -> dict:
    ...
    payload = {
        ...
        "full_name": full_name,
        "is_active": True,
        "is_admin": tg_user.id == ADMIN_TELEGRAM_ID,
    }
    created = supabase.table("bot_users").insert(payload).execute().data
    return created[0]

def get_or_create_day(user_id: int, norm_date: date) -> dict:
    rows = (
    ...
    created = supabase.table("daily_norms").insert(payload).execute().data
    return created[0]
...
async def show_day(callback: CallbackQuery, norm_date: date):
    ...
    text = (
        f"⚔️ <b>День {norm_date.strftime('%d.%m.%Y')}</b>\n\n"
        f"Набрано баллов: <b>{fmt_points(points)}</b>\n\n"
...
async def send_day_after_notification(callback: CallbackQuery):
    user = get_or_create_user(callback.from_user)
    norm_date = today()
    row = get_or_create_day(user["id"], norm_date)
    points = calc_points(row)
    ...
    await callback.message.answer(
        text,
        reply_markup=day_keyboard(row, norm_date),
    )
...
async def start(message: Message):
    ...
    greeting = (
        "⚔️ <b>ВОУ! ПРИВЕТСТВУЮ ТЕБЯ В ЛИГЕ КРАСАВЧИКОВ.</b>\n\n"
        "Здесь собираются те, кто не ищет лёгких путей.\n\n"
        "Каждый день — новый поход.\n"
        "Каждый выполненный норматив — балл в твою копилку.\n"
        "А в конце лучшие воины дружины заберут свою добычу 💰\n\n"
        "🛡 <b>Держи строй.</b>\n"
    ...
    await message.answer(greeting, reply_markup=menu_kb())
...
@router.callback_query(F.data == "ack_rules_update")
async def acknowledge_rules_update(callback: CallbackQuery):
    ...
    await send_day_after_notification(callback)
...
async def admin(message: Message):
    ...
    await message.answer(
        "👑 <b>Админка</b>\n\n"
        f"Участников: <b>{users.count or 0}</b>\n"
        f"Заполненных дней: <b>{norms.count or 0}</b>"
    )
...
async def choose_date(callback: CallbackQuery):
    ...
    await safe_edit(
        callback,
        "📜 <b>Выбери день</b>",
        month_calendar_kb(current_day.year, current_day.month),
    )
...
async def calendar_nav(callback: CallbackQuery):
    ...
    min_month = date(COMPETITION_START.year, COMPETITION_START.month, 1)

    current_day = today()
    max_month = date(current_day.year, current_day.month, 1)
...
async def date_selected(callback: CallbackQuery):
    ...
    await show_day(callback, selected_date)
...
async def toggle_norm(callback: CallbackQuery):
    ...
    norm_date = date.fromisoformat(date_str)
    ...
    row = get_or_create_day(user["id"], norm_date)
    new_value = not bool(row.get(field))
    ...
    row = updated_rows[0] if updated_rows else {**row, field: new_value}
    points = calc_points(row)
...
async def rating(callback: CallbackQuery):
    ...
    current_user = get_or_create_user(callback.from_user)
    ...
    daily_rows = (
        supabase.table("daily_norms")
        .select("*")
        .gte("norm_date", COMPETITION_START.isoformat())
        .lte("norm_date", today().isoformat())
        .execute()
        .data
    )

    score_by_user = {user["id"]: 0.0 for user in users}

    for row in daily_rows:
        user_id = row["user_id"]
        score_by_user[user_id] = score_by_user.get(user_id, 0.0) + calc_points(row)

    ranked = sorted(
        users,
        key=lambda user: (
            -score_by_user.get(user["id"], 0.0),
    ...
    for place in range(1, 11):
        ...
        if place <= len(ranked):
            user = ranked[place - 1]
            score = score_by_user.get(user["id"], 0.0)

            if user["id"] == current_user["id"]:
                shown_name = "⚔️ " + user["full_name"]
            else:
                shown_name = "Скрыто"

            score_text = fmt_points(score)
        else:
            shown_name = "Скрыто"
            score_text = "0"

        lines.append(
            f"{place_label(place)} {shown_name} — "
            f"<b>{score_text}</b> б. — 💰 {prize_text} ₽"
        )

    lines.append("")
    lines.append("⚔️ <b>ТВОЯ ПОЗИЦИЯ</b>")

    if my_place is not None:
        lines.append(f"Место: <b>{my_place}</b>")
        lines.append(f"Баллы: <b>{fmt_points(my_score)}</b>")
